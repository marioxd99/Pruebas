 ...
 * Tipo: Clase
 * Funci�n: Clase principal encargada de construir el laberinto mediante el algoritmo de Wilson
 */
public class GeneradorLaberinto {
	...
	int[] direccion = {0, 1, 2, 3}; // Vector que representa cada direcci�n. 0 = N, 1 = E, 2 = S, 3 = O
	private static final int TEMPORALIDAD = 20;
	...
	/* Nombre: run
	 * Tipo: M�todo
	 * Funci�n: Ejecutar construcci�n de JFrame seg�n par�metros
	...
	public void run() throws IOException {
		...
		int[] parametros; // Array que recoger� anchura y altura del laberinto
		...
		for (int i = 0; i < laberinto.length; i++) {
			for (int j = 0; j < laberinto[i].length; j++) {
				laberinto[i][j] = new Celda(j,i); // j = posici�n X en el dibujo e i = posici�n Y en el dibujo
			}
		}
	...
	/* Nombre: comenzar
	 * Tipo: M�todo
	 * Funci�n: B�squeda de primera celda aleatoria, selecci�n de siguiente celda aleatoria y llamada a procedimiento
	 * de b�squeda
	 */
	private void comenzar(Celda[][] laberinto) {
		...
		while(siguiente != null) {
			...
			if(contador == laberinto.length*laberinto[0].length) {                      // Si el n�mero de celdas blancas coincide con el n�mero de celdas total, fin
				break;
			} else {
	...
	private void posibleCamino(int f, int c, Celda[][] laberinto) {
		...
		ArrayList<Celda> camino = new ArrayList<Celda>(); // ArrayList que almacenar� celdas del posible camino
		ArrayList<Integer> movimientosPrevios = new ArrayList<Integer>();
		
		camino.add(laberinto[f][c]);

		laberinto[f][c].setColor(Color.RED); // Primeramente, el posible camino se mostrar� en rojo
		
		movimientosPrevios.add(-1); // Para primera comprobaci�n de movimientos previos
		laberinto[f][c].setDirHastaCelda(-1);

		do {
			ArrayList<Integer> candidatas = new ArrayList<Integer>(); // ArrayList (renovado en cada iteraci�n) que almacena los n�meros representativos de direcciones candidatas
			...
			camino.add(laberinto[f][c]); // A�adimos la siguiente celda vecina aleatoria al camino
			...
			if(laberinto[f][c].getColor().equals(Color.WHITE)) {
				for (Celda celda : camino) {
					...
					if(celda.getDirHastaCelda()>=0) {
						if(celda.getDirHastaCelda()==0) {                                               // El atributo que indica la direcci�n que hemos tomado para escoger esa celda nos ayuda a quitar los muros correspondientes
			...
			} else if(laberinto[f][c].getColor().equals(Color.RED)) {                                   // Si hemos a�adido al camino una celda color ROJO...
				
				int indiceRetorno = 0;                                                          // Variable que representa el punto de partida tras haber resuelto el bucle en cuesti�n
				
				for (int i=0; i < camino.size(); i++) {                                                     // La celda se debe encontrar consigo misma en un �ndice menor del ArrayList
					if(camino.get(i).getFila()==laberinto[f][c].getFila() && 
                                                camino.get(i).getColumna()==laberinto[f][c].getColumna()) {
						indiceRetorno = i;
						break;
					}
				}
				...
				movimientosPrevios.add(mover); // A�adimos cifra identificadora de movimiento al ArrayList movimientosPrevios
				generadorFrame.repintar(); // Repintamos para poder visualizar la nueva celda color rojo
			}
			
			try {
				Thread.sleep(TEMPORALIDAD); // Ayudar� a construir m�s deprisa o m�s lento el laberinto
	 ...
	 * Tipo: M�todo
	 * Funci�n: Seleccionar aleatoriamente una nueva celda de inicio de camino
	 */
